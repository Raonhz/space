import { ISSData, LocationData, WeatherData } from '../types.ts';
...
export const fetchLocationDetails = async (lat: number, lon: number): Promise<LocationData> => {
  try {
    ...
    if (data.error) {
      // Likely over the ocean if no address is found
      return {
...
export const fetchWeather = async (lat: number, lon: number): Promise<WeatherData | null> => {
  try {
    ...
    // Simple mapping of WMO weather codes to readable strings
