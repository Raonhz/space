...
const InfoOverlay: React.FC<InfoOverlayProps> = ({ state }) => {
  ...
  return (
    <div className="absolute bottom-0 left-0 w-full p-3 sm:p-6 z-30 flex flex-col gap-2 sm:gap-4">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-end justify-between">
        ...
        {/* Location & Coordinates with Title */}
          ...
          <div className="bg-black/50 backdrop-blur-xl border border-white/10 rounded-2xl p-3.5 sm:p-5 shadow-2xl">
            <div className="flex items-start justify-between mb-2 sm:mb-4">
              <div>
                ...
                <h2 className="text-xl sm:text-3xl font-light text-white tracking-wide flex items-center gap-2 sm:gap-3">
                  <MapPin className="text-blue-400 shrink-0" size={18} />
                  <span className="truncate">{location?.name || 'CALIBRATING COORDINATES...'}</span>
                </h2>
                <p className="text-white/60 text-xs sm:text-sm mt-1 ml-7 sm:ml-9">
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-4 pt-2.5 sm:pt-4 border-t border-white/10">
              <div>
                <div className="text-white/40 text-[8px] sm:text-[10px] font-mono uppercase tracking-wider mb-0.5">LATITUDE</div>
                <div className="text-white font-mono text-xs sm:text-sm truncate">{formatCoord(iss.latitude, true)}</div>
              </div>
              <div>
                <div className="text-white/40 text-[8px] sm:text-[10px] font-mono uppercase tracking-wider mb-0.5">LONGITUDE</div>
                <div className="text-white font-mono text-xs sm:text-sm truncate">{formatCoord(iss.longitude, false)}</div>
              </div>
              <div>
                <div className="text-white/40 text-[8px] sm:text-[10px] font-mono uppercase tracking-wider mb-0.5">ALTITUDE / ORBITAL VELOCITY</div>
        ...
        {/* Space Facts Ticker */}
        <div className="hidden sm:block bg-black/50 backdrop-blur-xl border border-white/10 rounded-2xl p-4 w-full md:w-[420px] h-36 flex flex-col shadow-2xl overflow-hidden relative">
          <style>{`
             ...
             .animate-fact-cycle {
               animation: fact-fade-cycle 8s cubic-bezier(0.25, 1, 0.5, 1) forwards;
             }
           `}</style>

          {/* 헤더: 카테고리 태그 포함 */}
          <div className="flex items-center justify-between mb-2">
            <div className="text-white/40 text-[10px] font-mono uppercase tracking-wider flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-yellow-500 animate-pulse"></div>
              SPACE INTELLIGENCE
            </div>
            {currentFact && (
              <div className="text-[8px] font-mono uppercase tracking-wider px-2 py-0.5 rounded-full bg-cyan-500/15 text-cyan-400/80 border border-cyan-500/20">
          ...
          <div className="flex-1 flex flex-col justify-center pr-2">
            {currentFact ? (
              <div
                key={`${currentFact.id}-${activeFactIndex}`}
                className="animate-fact-cycle"
              >
                <div className="text-white/95 text-[11px] sm:text-xs leading-relaxed border-l-2 border-cyan-500/50 pl-3.5 py-0.5">
                ...
                <div className="text-white/25 text-[8px] font-mono mt-1.5 pl-5 tracking-wider">
